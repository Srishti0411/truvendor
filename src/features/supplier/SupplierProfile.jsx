...
const SupplierProfile = () => {
  ...
  const handleAddProduct = () => {
    if (newProduct && newRate) {
      setProducts([...products, { name: newProduct, rate: Number(newRate) }]);
      setNewProduct("");
      setNewRate("");
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-blue-50">
      <div className="bg-white p-8 rounded-xl shadow-md w-full max-w-2xl">
        ...
        <p className="text-sm text-center text-gray-500 mb-4">Update your name, location and the products you sell.</p>
        ...
        <ul className="mb-4">
          {products.map((item, i) => (
            <li key={i} className="flex justify-between border-b py-2 text-gray-700">
              <span>{item.name}</span>
